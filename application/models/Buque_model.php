...
class Buque_model extends CI_model
{
    ...
    public function get_desin()
    {
        $this->db->select('b.id, b.matricula, b.nombrebuque, e.id_status, b.observacion, b.fecha_desincorporacion,
                       CASE
                           WHEN b.desincorporar = 0 THEN \'Desincorporar\'
                           WHEN b.desincorporar = 1 THEN \'Incorporado\'
                           ELSE \'Desconocido\' -- Optional: handle other values if any
                       END AS estado_desincorporacion'); // Alias for the new column
        $this->db->from('public.buque b');
        $this->db->join('mensualidad e', 'e.matricula = b.matricula', 'left');
        $this->db->where('e.id_status =', '2');
        $this->db->group_by("b.matricula, b.nombrebuque, e.id_status, b.fecha_desincorporacion, b.observacion, b.id, b.desincorporar"); // Include desincorporar in GROUP BY
        $this->db->order_by('fecha_desincorporacion asc');

        $query = $this->db->get();

        if ($query->num_rows() > 0) { // Using num_rows() is generally preferred
            return $query->result();
        } else {
            return false; // Or an empty array, depending on your desired behavior
        }
    }
