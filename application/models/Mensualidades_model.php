...
class Mensualidades_model extends CI_model
{
    ...
    public function ver_todos2($filters = [])
    {
        // Seguridad: si llegan vacíos, no devolver nada
        if (empty($filters['matricula']) && (empty($filters['desde']) || empty($filters['hasta']))) {
            return [];
        }
    ...
    ');
        ...
        if (!empty($filters['matricula'])) {
            $this->db->where('m.matricula', $filters['matricula']);
        }
        if (!empty($filters['desde']) && !empty($filters['hasta'])) {
            $this->db->where('m.fecha_deuda >=', $filters['desde']);
            $this->db->where('m.fecha_deuda <=', $filters['hasta']);
        }
    ...
    public function guardar_proc_pag($data)
    {
        ...
        if ($response) {
            ...
            $data1 = array(
                'id_mensualidad'     => $data['id_mesualidad_ver'],
